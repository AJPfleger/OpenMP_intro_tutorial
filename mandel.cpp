...
void testpoint(const int max_iter = 1000){
  struct d_complex z;
  int iter;
  double temp;
...
//  const
  ...
  const double area = 2.0 * 2.5 * 1.125 *
      static_cast<double>(n_points*n_points-numoutside) / static_cast<double>(n_points * n_points);
  double error = area / (double)n_points;
